    ...
    <div v-if="!loading && !loadingAncestors">
      <nav class="p-1 mb-3">
        <div class="flex justify-end items-center gap-2 mb-3">
          ...
          <ol class="inline-flex items-center justify-start space-x-1 md:space-x-3">
              <router-link :to="{ name: 'dashboard' }" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                      <path fill-rule="evenodd" d="M9.293 2.293a1 1 0 0 1 1.414 0l7 7A1 1 0 0 1 17 11h-1v6a1 1 0 0 1-1 1h-2a1 1 0 0 1-1-1v-3a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v3a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-6H3a1 1 0 0 1-.707-1.707l7-7Z" clip-rule="evenodd" />
                  </svg>
                  <span class="whitespace-nowrap ml-1">My Files</span>
              </router-link>
              <li v-for="(ans,i) of ancestors" :key="ans.id"  class="inline-flex items-center">
                  <div class="flex items-center" v-if="i != 0">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                          <path fill-rule="evenodd" d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" />
                      </svg>
                      <router-link :to="{ name: 'dashboard' , query: { parent_id: ans.id }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">
                          {{ ans.name }}
                      </router-link>
                  </div>
              </li>
          ...
          <table class="min-w-full">
              <thead class="bg-gray-100 border-b border-gray-300 shadow ">
                  <tr>
                      ...
                      <th class="hidden md:table-cell text-sm font-medium text-gray-900 px-6 py-4 text-left">
                          Last Modified
                      </th>
                      <th class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                          Size
...
import { AddFavorite, AddToMyFavorites, DownloadFiles, emitter, Notification, OpenFile, ReloadFiles, RemoveFavorites, ShareFiles, ShowDeleteFileConfirmation } from '@/event-bus';
...
onMounted(() => {
    ...
    router.beforeEach((to, from) => {
        if((from.name == 'login' || from.name == 'verify-email')){
