        ...
        <nav class="p-1 mb-3">
            ...
            <ol class="inline-flex items-center justify-start">
                <li class="inline-flex items-center me-1">
                    <router-link :to="{ name: 'shared-by-me' }" class="gap-2 inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186 9.566-5.314m-9.566 7.5 9.566 5.314m0 0a2.25 2.25 0 1 0 3.935 2.186 2.25 2.25 0 0 0-3.935-2.186Zm0-12.814a2.25 2.25 0 1 0 3.933-2.185 2.25 2.25 0 0 0-3.933 2.185Z" />
                        </svg>

                        <span class="whitespace-nowrap ml-1">Shared by me</span>
                    </router-link>
                </li>
                <li v-for="ans,index of ancestors" :key="ans.id" class="inline-flex items-center">
                    <div class="flex items-center" v-if="index != 0">
                        ...
                        <router-link :to="{ name: 'shared-by-me', query: { parent_id: ans.id }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">
                            {{ ans.name }}
                        </router-link>
                    </div>
                </li>
...
import { AddFavorite, AddToMyFavorites, DownloadFiles, emitter, Notification, OpenFile, ReloadFiles, RemoveFavorites, ShareFiles } from '@/event-bus';
...
const getSharedByMeFiles = async () => {
    loading.value = true
    await api.post('get-shared-files-by-me',
    {
...
const deleteSelected = () => {
    if(Object.keys(selected.value).length){
        deleteFromShareByMe(Object.keys(selected.value))
    }
}
const deleteFile = (id) => {
    deleteFromShareByMe([id])
} 

const deleteFromShareByMe = async (ids) => {
    await api.post('delete-share-by-me',
    {
        ids: ids
    },
